...
double getResultS(double x, double e, double y, int& n) { // Ссылка на переменную n
	double sum = 0;
	double term = -1;
	n = 0;
	do {
		++n;
		term *= -x * x; // Вычисление следующего члена ряда
		sum += term / (2 * n * (2 * n - 1)); // Добавление текущего члена ряда к сумме
	} while (fabs(sum - y) >= e); // Цикл выполняется, пока абсолютное значение текущего члена ряда больше или равно заданной погрешности e
